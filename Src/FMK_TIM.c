...
t_eReturnCode FMKTIM_Init(void)
{
    ...
    t_uint16 timClkValueMHz_u16 = (t_uint16)0;
    t_uint16 ahbClkValueMHz_u16 = (t_uint16)0;
    t_uint16 apb1ClkValueMHz_u16 = (t_uint16)0;
    t_uint16 apb2ClkValueMHz_u16 = (t_uint16)0;
    t_uint8 timClkMultiplier_u8 = (t_uint8)1;
    t_eFMKCPU_SysClkOsc oscTimerSrc_e;
    t_sFMKTIM_TimerInfo * timerInfo_ps;
    t_sFMKTIM_ChnlInfo * chnlInfo_ps;

    Ret_e = FMKCPU_GetSysClkValue(FMKCPU_SYS_CLOCK_AHB1, &ahbClkValueMHz_u16);
    if(Ret_e == RC_OK)
    {
        Ret_e = FMKCPU_GetSysClkValue(FMKCPU_SYS_CLOCK_APB1, &apb1ClkValueMHz_u16);
    }
    if(Ret_e == RC_OK)
    {
        Ret_e = FMKCPU_GetSysClkValue(FMKCPU_SYS_CLOCK_APB2, &apb2ClkValueMHz_u16);
    }
    if(Ret_e == RC_OK)
    {
        FMKSRL_LOG("[TIM] HCLK=%uMHz APB1=%uMHz APB2=%uMHz\r\n",
                   ahbClkValueMHz_u16,
                   apb1ClkValueMHz_u16,
                   apb2ClkValueMHz_u16);
    }
    ...
    if(Ret_e == RC_OK)
    {
        for (timIndex_u8 = (t_uint8)0 ; (timIndex_u8 < (t_eFMKTIM_Timer)FMKTIM_TIMER_NB) && (Ret_e == RC_OK) ; timIndex_u8++)
        {
            ...
            if(Ret_e == RC_OK)
            {
                Ret_e = FMKTIM_Get_TimerClockMultiplierSpecific(timerInfo_ps->c_clock_e,
                                                                timClkValueMHz_u16,
                                                                ahbClkValueMHz_u16,
                                                                &timClkMultiplier_u8);
            }
            if(Ret_e == RC_OK)
            {
                timerInfo_ps->timerFreqMHz_u32 = (t_uint32)((t_uint32)timClkMultiplier_u8 *
                                                            (t_uint32)timClkValueMHz_u16);
                FMKSRL_LOG("[TIM] idx=%u clkPort=%u src=%uMHz mul=%u eff=%luMHz\r\n",
                           timIndex_u8,
                           timerInfo_ps->c_clock_e,
                           timClkValueMHz_u16,
                           timClkMultiplier_u8,
                           (t_uint32)timerInfo_ps->timerFreqMHz_u32);
        ...
        && (GETBIT(f_mask_u8, FMKTIM_BIT_IC_FREQUENCY) == BIT_IS_SET_8B))
        {
            Ret_e = c_FMKTIM_TimerFunc_apf[FMKTIM_HWTIM_CFG_IC].
                    GetTimerInfoInit_pcb(timerInfo_s.c_clock_e,
                                        ...
                                        (t_float32)f_ICOpe_s.frequency_f32,
                                        &bspARRVal_u32,
                                        &bspPSCVal_u32);
            ...
            if(Ret_e == RC_OK)
            {
                ...
                //---- trigger an even to update the instance value directly ----//
                timerInfo_s.bspTimer_ps->Instance->EGR = TIM_EGR_UG;
            }
            else
            {
                ASSERT((t_sint32)Ret_e);
    ...
    {   
        ...
        if (Ret_e == RC_OK)
        {
            ...
            if(bspChannelState_e == HAL_TIM_CHANNEL_STATE_READY)
            {
                ...
                Ret_e = FMKTIM_Set_OcChannelSpecificCfg(&BspOcInit_s);
                                                ...
                                                t_uint32 * f_ArrValue_pu32)
{
    ...
    if(Ret_e == RC_OK)
    {
        switch(f_EcdrMode_e)
        {
            ...
            default:
            {
                Ret_e = FMKTIM_Get_EcdrARRValueSpecific(f_EcdrMode_e,
                                                        f_rqstArrValue_u32,
                                                        f_ArrValue_pu32);
                if(Ret_e != RC_OK)
                {
...
static t_eReturnCode s_FMKTIM_Get_BspChannel(t_eFMKTIM_InterruptChnl f_channel_e, t_uint32 *f_bspChnl_pu32)
{
    ...
    if (Ret_e == RC_OK)
    {
        ...
        default:
            Ret_e = FMKTIM_Get_BspChannelSpecific(f_channel_e, f_bspChnl_pu32);
            if(Ret_e != RC_OK)
            {
...
static t_eReturnCode s_FMKTIM_GetBspEcdrMode(t_eFMKTIM_EcdrMode f_EcdrMode_e, t_uint32 *f_bspEcdrMode_pu32)
{
    ...
    if (Ret_e == RC_OK)
    {
        switch (f_EcdrMode_e)
        {
            ...
            case FMKTIM_ECDR_MODE_NB:
            default:
                Ret_e = FMKTIM_Get_BspEcdrModeSpecific(f_EcdrMode_e, f_bspEcdrMode_pu32);
                if(Ret_e != RC_OK)
                {
                    Ret_e = RC_ERROR_NOT_SUPPORTED;
                }
                                                    ...
                                                    t_float32 f_timFreq_f32)
{
    ...
    else if(f_timFreq_f32 < FMKTIM_TIMER_MIN_FREQ_ALLOWED)
    {
        Ret_e = RC_ERROR_PARAM_INVALID;
    }
    if(Ret_e == RC_OK)
